...
class plgJCommentsAvatar extends JPlugin
{
	...
	$sessiontime = time() - abs((int)$this->params->get('sessiontime', 15)) * 60 - 1;
    $db->setQuery('SELECT user_id, profile_value as avatar, userid as online, time as usertime FROM `#__user_profiles` '.
    ...
    foreach ($comments as &$comment) {
     ...
     if (isset($avatars[$uid]) && !empty(trim($avatars[$uid]->avatar))) {
	  ...
	  $comment->sessiontime = $sessiontime;
     }
    }
